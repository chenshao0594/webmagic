...
class ModelPageProcessor implements PageProcessor {
	private static Logger LOGGER = LoggerFactory.getLogger(ModelPageProcessor.class);
	...
	public static ModelPageProcessor create(SiteConfig site, Class... clazzs) {
	...
	private ModelPageProcessor(SiteConfig site) {
		this.site = site;
	}
	...
	public void process(Page page) {
		for (PageModelExtractor pageModelExtractor : pageModelExtractorList) {
			...
			if (process == null) {
				LOGGER.info("the object is null, continue ...");
				continue;
			}
			if ((process instanceof List && ((List) process).size() == 0)) {
				LOGGER.info("the object is List continue ...");
				continue;
			}
	...
	protected void postProcessPageModel(Class clazz, Object object) {
		LOGGER.info("post  PageModel process ....");
	}
